            ...
            @if ($items->count() > 0)
                ...
                <div class="flex space-x-4 sm:space-x-6 md:space-x-8 overflow-x-auto pb-4 px-4 scrollbar-hide ">
                    @foreach ($items as $item)
                        <form method="POST" action="{{ route($route, $item) }}" class="bg-white rounded-2xl shadow-lg group animate__animated animate__fadeInUp flex-none" style="animation-delay: {{ $loop->index * 0.15 }}s;">
                            @csrf
                                ...
                                <div class="relative  rounded-xl overflow-hidden shadow-2xl transform transition-all duration-500 w-32 h-32 sm:w-48 sm:h-48 md:w-64 md:h-64 lg:w-80 lg:h-80">
                                    ...
                                    <div class="w-full h-full relative overflow-hidden">
                                        @if($item->profile_photo_path)
                                            <!-- Team Image -->
                                            <img src="{{ $item->profile_photo_url }}" style="object-fit: scale-down;" alt="{{ $item->name }}" class="w-full h-full object-cover animate__animated animate__fadeIn transition-transform duration-500 group-hover:scale-110">
                                        @else
