...
<div class="{{ $card }}" @if($tooltip) title="{{ $tooltip }}" @endif aria-disabled="{{ $disabled ? 'true' : 'false' }}">
    @if($label && config('app.debug'))
        <div class="mb-3">
            <span class="inline-flex items-center rounded-full bg-gray-200 text-gray-700 px-2 py-0.5 text-xs" title="Conditions d'accès">
                🔒 {{ $label }}
            </span>
        </div>
    @endif
    ...
    @if($image)
        <div class="mt-auto self-center">
            <img style="height:100px;width:100px;object-fit:scale-down;" src="{{ $image }}" alt="{{ strip_tags($titre) }}" class="w-full h-auto rounded-lg">
            <br>
        </div>
    @endif
    ...
    <div class="mt-auto self-start">
        ...
        @else
            <span class="bg-gray-300 text-gray-600 font-semibold py-3 px-6 rounded-lg inline-block pointer-events-none select-none">
                {!! $button !!}
            </span>
        @endif
    </div>
