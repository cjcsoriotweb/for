<div class="flex h-full flex-col text-white">
    @if ($conversationId && trim($systemPrompt) !== '')
            ...
            <header class="border-b border-white/10 px-6 py-5 sm:px-8">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="min-w-0 space-y-2">
                        ...
                        <p class="text-lg font-semibold text-slate-100 sm:text-xl">
                            {{ __('Session de conversation') }}
                        </p>
                        ...
                        <details class="group mt-2">
                            <summary class="inline-flex cursor-pointer list-none items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-200 transition hover:border-emerald-400/80 hover:bg-emerald-500/20">
                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                    <path d="M6 9l6 6 6-6"></path>
                                </svg>
                                <span>{{ __('Voir le prompt') }}</span>
                            </summary>
                            <pre class="mt-2 max-h-48 overflow-auto rounded-2xl border border-white/10 bg-slate-900/70 p-3 text-xs text-slate-200 whitespace-pre-wrap">{{ $systemPrompt }}</pre>
                        </details>
                    </div>
                    ...
                    <div id="assistantChatMessages" class="flex flex-1 flex-col space-y-4 overflow-y-auto pb-4 pr-1 sm:pr-2">
                        @if ($awaitingResponse)
                            <div class="flex items-center gap-2 self-start rounded-3xl bg-emerald-400/10 px-4 py-2 text-sm text-emerald-200 shadow-lg shadow-emerald-900/50 backdrop-blur">
                                ...
                                <span>{{ __("L'assistant prépare sa réponse...") }}</span>
                            </div>
                        @endif
                        ...
                        @empty
                            <div class="flex flex-1 flex-col items-center justify-center text-center text-slate-300">
                                <div class="py-10">
                                    ...
                                    <p class="text-sm text-slate-300">{{ __('Posez votre première question pour lancer le dialogue.') }}</p>
                                </div>
                            </div>
                        @endforelse
                    </div>
            ...
            <div class="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 px-8 py-10 text-center shadow-xl shadow-slate-950/40 backdrop-blur">
                ...
                @else
                    ...
                    <p class="text-sm text-slate-300">
                        {{ __("L'assistant IA n'est pas configuré (prompt manquant) ou la conversation n'a pas pu être initialisée.") }}
                    </p>
                    <p class="mt-2 text-xs text-slate-400">
                        {{ __("Astuce : passez le prompt via la query string ?prompt=... ou via mount(\$prompt).") }}
                    </p>
                @endif
            </div>
        </div>
