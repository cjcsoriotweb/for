    ...
    <div class="flex h-full flex-1 flex-col">
        <div class="border-b border-slate-200 bg-white px-6 py-4">
            <div class="flex items-center gap-3">
                ...
                @endif
                <div class="min-w-0">
                    <h3 class="truncate text-lg font-semibold text-slate-900">{{ $trainerName ?: __('Assistant IA') }}</h3>
                    @if ($trainerDescription)
                        <p class="truncate text-sm text-slate-600">{{ $trainerDescription }}</p>
                    @else
                        <p class="truncate text-sm text-slate-600">{{ __('Votre assistant IA personnel pour toutes vos questions.') }}</p>
                    @endif
                </div>
            </div>
        </div>

        <div class="flex flex-1 flex-col overflow-hidden" wire:poll.1s="poll">
                ...
                @foreach ($messages as $message)
                    <div class="flex {{ $message['role'] === 'assistant' ? 'justify-start' : 'justify-end' }}">
                        <div class="max-w-full rounded-xl px-4 py-3 shadow-sm {{ $message['role'] === 'assistant' ? 'bg-white' : 'bg-emerald-500' }}">
                            ...
                            <div class="prose prose-sm max-w-none break-words {{ $message['role'] === 'assistant' ? 'text-slate-900 prose-headings:text-slate-900 prose-a:text-emerald-600' : 'text-white prose-headings:text-white prose-a:text-white/90' }}">
    ...
    <div class="flex h-full flex-1 items-center justify-center bg-slate-50">
        <div class="max-w-md rounded-2xl border border-slate-200 bg-white px-8 py-10 text-center shadow-sm">
            ...
            @else
                ...
                <p class="text-slate-600">{{ __('L\'assistant IA n\'est pas configuré ou n\'est pas disponible pour le moment.') }}</p>
            @endif
        </div>
    </div>
@endif
