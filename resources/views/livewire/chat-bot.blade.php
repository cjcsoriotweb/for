<div>
    ...
    <div class="pointer-events-none fixed inset-0 z-[9999] flex items-end justify-end p-5 sm:p-6">
        <button type="button" aria-label="Discuter avec l'assistant IA"
    ...
    <div id="assistant-modal"
        ...
        <div
            class="relative w-full max-w-5xl h-[80vh] rounded-3xl bg-white shadow-2xl shadow-slate-900/50 overflow-hidden">
            <button type="button"
                ...
                aria-label="Fermer le chat IA" data-assistant-close>
                ✕
            </button>

            <iframe src="{{ config('services.goodview_genie_url') }}?models= " title="Assistant IA"
                class="h-full w-full border-0" loading="lazy"></iframe>
        </div>
    </div>
    ...
    <script>
        (() => {
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', initAssistantModal, {
                    once: true
                });
            } else {
                initAssistantModal();
