<x-app-layout>
  ...
  @php
    ...
    $descriptionTips = [
        'Commencez par rappeler l objectif du quiz et son role dans la formation.',
    ...
    <div class="mx-auto max-w-4xl space-y-8 sm:px-6 lg:px-8">
      <nav class="flex text-sm text-white/80" aria-label="Fil d Ariane">
        <ol class="inline-flex items-center gap-2">
          ...
          <li class="text-gray-400">/</li>
          <li class="text-gray-500">Description du quiz</li>
        </ol>
      </nav>
        ...
        <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div class="max-w-xl">
            ...
            <p class="mt-2 text-sm text-white/80">
              Quelques phrases suffisent pour rassurer vos eleves, expliquer le but du quiz et annoncer les modalites.
            </p>
          </div>
          <div class="grid w-full max-w-xs gap-3 text-sm text-white">
            ...
            <div class="rounded-2xl bg-white/15 p-4 backdrop-blur">
              <p class="text-xs uppercase tracking-wide text-white/70">Description</p>
              <p class="mt-1 text-base font-semibold">
                {{ $quiz->description ? 'Redigee' : 'A definir' }}
              </p>
            </div>
          </div>
        </div>
      ...
      <section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_260px]">
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div class="border-b border-gray-100 px-6 py-5">
            <h2 class="text-xl font-semibold text-gray-900">Description clarifiante</h2>
            <p class="mt-1 text-sm text-gray-600">
              Donnez le contexte, les regles et l esprit du quiz avant que vos eleves ne se lancent.
            </p>
          </div>
          <div class="space-y-6 px-6 py-6">
            <div class="rounded-xl border border-indigo-100 bg-indigo-50/70 p-4 text-sm text-indigo-900">
              <p>
                Structure conseillee : objectif du quiz, consignes pratiques, conseil final (par exemple rappeler le seuil de reussite).
              </p>
            </div>
            ...
            <form method="POST" action="{{ route('formateur.formation.chapter.lesson.quiz.update', [$formation, $chapter, $lesson]) }}" class="space-y-6">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Suggere : 2 ou 3 phrases pour informer et motiver.</p>
                <textarea
                  id="quiz_description"
                  name="quiz_description"
                  rows="6"
                  class="mt-3 w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200 @error('quiz_description') border-red-500 focus:border-red-500 focus:ring-red-200 @enderror"
                  placeholder="Exposez le but du quiz, les consignes et les attentes en quelques phrases..."
        ...
        <div class="space-y-5">
          ...
          <div class="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-5 text-sm text-indigo-900 shadow-sm">
            <h3 class="text-base font-semibold text-indigo-900">Conseils de redaction</h3>
            <ul class="mt-3 space-y-2">
              @foreach ($descriptionTips as $tip)
