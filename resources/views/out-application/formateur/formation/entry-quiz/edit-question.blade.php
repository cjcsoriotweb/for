@php
    ...
    if (empty($initialChoices)) {
        $initialChoices = $choices->toArray();
    }
@endphp
...
<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-2xl font-semibold text-white">Modifier la question</h1>
                </div>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="px-6 py-6">
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Mettez à jour le contenu de la question et ses réponses.
                    </p>
                </div>

                <div class="border-t border-slate-200 px-6 py-6">
                    <form id="question-form" method="POST"
                        action="{{ route('formateur.formation.entry-quiz.questions.update', [$formation, $question]) }}"
                        class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea id="question_text" name="question_text" rows="3"
                                ...
                                placeholder="Saisissez la question">{{ old('question_text', $question->question) }}</textarea>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="question_type" name="question_type"
                                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="multiple_choice" @selected(old('question_type', $question->type) === 'multiple_choice')>
                                    Choix multiple
                                </option>
                                <option value="true_false" @selected(old('question_type', $question->type) === 'true_false')>
                                    Vrai / Faux
                                </option>
                            </select>
                        </div>
    ...
    <script>
        (() => {
            ...
            function createChoiceRow(choice, index) {
                ...
                row.innerHTML = `
                    <div class="flex-1 space-y-1">
                        <label class="text-xs font-medium text-slate-500">Réponse ${index + 1}</label>
            ...
            function syncWithType() {
                if (typeSelect.value === 'true_false') {
                    const choices = getCurrentChoices();
                    const limited = choices.slice(0, 2);
                    while (limited.length < 2) {
                        limited.push({ text: '', is_correct: limited.length === 0 });
                    }
                    renderChoices(limited);
            ...
            form.addEventListener('submit', (event) => {
                ...
                if (choices.length < 2) {
                    ...
                    alert('Veuillez renseigner au moins deux réponses.');
                    return;
                }
                ...
                if (!hasText) {
                    event.preventDefault();
                    alert('Veuillez remplir le texte de chaque réponse.');
                    return;
                }
