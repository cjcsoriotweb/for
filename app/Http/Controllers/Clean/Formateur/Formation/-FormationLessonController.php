...
class FormationLessonController
{
    public function createLesson(Formation $formation, Chapter $chapter, FormationService $formationService)
    {
        ...
        return redirect()->route('formateur.formation.chapters.index', $formation)->with('success', 'LeÃ§on crÃ©Ã©e avec succÃ¨s.');
    }
    ...
    public function deleteLesson(Formation $formation, Chapter $chapter, Lesson $lesson, FormationService $formationService)
    {
        try {
            ...
            return redirect()->route('formateur.formation.chapters.index', $formation)
                ->with('success', 'LeÃ§on supprimÃ©e avec succÃ¨s.');
        } catch (\Exception $e) {
            return back()->withErrors(['error' => 'Erreur lors de la suppression de la leÃ§on: '.$e->getMessage()]);
        }
    }
    ...
    public function defineLesson(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        if (! $lessonType) {
            return back()->withErrors(['lesson_type' => 'Veuillez sÃ©lectionner un type de leÃ§on.']);
        }
        ...
        switch ($lessonType) {
            case 'quiz':
                return redirect()->route('formateur.formation.chapter.lesson.quiz.create', [$formation, $chapter, $lesson])
                    ->with('success', 'Type de leÃ§on dÃ©fini. Vous pouvez maintenant crÃ©er votre quiz.');

            case 'video':
                return redirect()->route('formateur.formation.chapter.lesson.video.create', [$formation, $chapter, $lesson])
                    ->with('success', 'Type de leÃ§on dÃ©fini. Vous pouvez maintenant ajouter votre vidÃ©o.');

            case 'text':
                return redirect()->route('formateur.formation.chapter.lesson.text.create', [$formation, $chapter, $lesson])
                    ->with('success', 'Type de leÃ§on dÃ©fini. Vous pouvez maintenant ajouter votre contenu textuel.');

            default:
                return back()->withErrors(['lesson_type' => 'Type de leÃ§on invalide.']);
        }
    }
    ...
    public function storeQuiz(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        try {
            ...
            return redirect()->route('formateur.formation.chapters.index', [$formation,$chapter])
                ->with('success', 'Quiz crÃ©Ã© avec succÃ¨s! Vous pouvez maintenant ajouter des questions.');
        } catch (\Exception $e) {
            return back()->withInput()->withErrors(['error' => 'Erreur lors de la crÃ©ation du quiz: '.$e->getMessage()]);
        }
    }
    ...
    public function storeVideo(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        try {
            ...
            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'VidÃ©o ajoutÃ©e avec succÃ¨s!');
        } catch (\Exception $e) {
            return back()->withInput()->withErrors(['error' => 'Erreur lors de l\'ajout de la vidÃ©o: '.$e->getMessage()]);
        }
    }
    ...
    public function storeText(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        try {
            ...
            if (request()->hasFile('inline_document')) {
                $this->storeLessonResource($lesson, request()->file('inline_document'), 'inline');
            }
            ...
            if (is_array($downloadFiles)) {
                foreach ($downloadFiles as $file) {
                    if ($file instanceof UploadedFile) {
                        $this->storeLessonResource($lesson, $file, 'download');
                    }
                }
            }

            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'Contenu textuel crÃ©Ã© avec succÃ¨s!');
        } catch (\Exception $e) {
            return back()->withInput()->withErrors(['error' => 'Erreur lors de la crÃ©ation du contenu: '.$e->getMessage()]);
        }
    }
    ...
    public function editQuiz(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        if (! $quiz || ! ($quiz instanceof \App\Models\Quiz)) {
            return redirect()->route('formateur.formation.chapter.lesson.define', [$formation, $chapter, $lesson])
                ->withErrors(['error' => 'Quiz non trouvÃ© pour cette leÃ§on.']);
        }
    ...
    public function updateQuiz(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        try {
            ...
            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'Quiz modifiÃ© avec succÃ¨s!');
        } catch (\Exception $e) {
    ...
    public function storeQuestion(Formation $formation, Chapter $chapter, Lesson $lesson, \App\Models\Quiz $quiz)
    {
        ...
        try {
            ...
            if (! $choices || ! is_array($choices)) {
                return back()->withInput()->withErrors(['error' => 'Format des rÃ©ponses invalide.']);
            }
            ...
            foreach ($choices as $choiceIndex => $choiceData) {
                ...
                if (!isset($choiceData['text']) || trim($choiceData['text']) === '') {
                    ...
                    throw new \Exception('Le texte du choix ne peut pas Ãªtre vide (choix #' . ($choiceIndex + 1) . ')');
                }
                ...
                if (empty($choiceText)) {
                    ...
                    throw new \Exception('Le texte du choix ne peut pas Ãªtre vide aprÃ¨s nettoyage (choix #' . ($choiceIndex + 1) . ')');
                }
            ...
            return back()->with('success', 'Question ajoutÃ©e avec succÃ¨s!');
        } catch (\Exception $e) {
    ...
    public function updateQuestion(Formation $formation, Chapter $chapter, Lesson $lesson, \App\Models\Quiz $quiz, \App\Models\QuizQuestion $question)
    {
        ...
        if ($question->quiz_id !== $quiz->id) {
            return back()->withErrors(['error' => 'Question non trouvÃ©e pour ce quiz.']);
        }
        ...
        try {
            ...
            return back()->with('success', 'Question modifiÃ©e avec succÃ¨s!');
        } catch (\Exception $e) {
    ...
    public function deleteQuestion(Formation $formation, Chapter $chapter, Lesson $lesson, \App\Models\Quiz $quiz, \App\Models\QuizQuestion $question)
    {
        ...
        try {
            ...
            return back()->with('success', 'Question supprimÃ©e avec succÃ¨s!');
        } catch (\Exception $e) {
    ...
    public function editVideo(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        if (! $videoContent || ! ($videoContent instanceof \App\Models\VideoContent)) {
            return redirect()->route('formateur.formation.chapter.lesson.define', [$formation, $chapter, $lesson])
                ->withErrors(['error' => 'VidÃ©o non trouvÃ©e pour cette leÃ§on.']);
        }
    ...
    public function updateVideotodelete(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        try {
            ...
            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'VidÃ©o modifiÃ©e avec succÃ¨s!');
        } catch (\Exception $e) {
            return back()->withInput()->withErrors(['error' => 'Erreur lors de la modification de la vidÃ©o: '.$e->getMessage()]);
        }
    }
    ...
    public function editText(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        if (! $textContent || ! ($textContent instanceof TextContent)) {
            return redirect()->route('formateur.formation.chapter.lesson.define', [$formation, $chapter, $lesson])
                ->withErrors(['error' => 'Contenu textuel non trouvÃ© pour cette leÃ§on.']);
        }
    ...
    public function showResources(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        $lesson->load(['resources', 'lessonable']);

        $supportsInline = $lesson->lessonable_type === TextContent::class;
        $inlineAttachment = $supportsInline ? $lesson->resources->firstWhere('display_mode', 'inline') : null;
        $downloadAttachments = $lesson->resources->when($supportsInline, fn ($collection) => $collection->where('display_mode', 'download'), fn ($collection) => $collection);

        return view('out-application.formateur.formation.chapter.lesson.resources', [
            'formation' => $formation,
            'chapter' => $chapter,
            'lesson' => $lesson,
    ...
    public function updateText(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        ...
        $validated = request()->validate([
            ...
            'remove_attachments.*' => 'integer|exists:lesson_resources,id',
        ]);
        ...
        try {
            ...
            if (request()->boolean('remove_inline_document')) {
                $this->removeInlineResources($lesson);
            }
            ...
            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'Contenu textuel modifiÃ© avec succÃ¨s!');
        } catch (\Exception $e) {
            return back()->withInput()->withErrors(['error' => 'Erreur lors de la modification du contenu: '.$e->getMessage()]);
        }
    }

    public function storeResources(Formation $formation, Chapter $chapter, Lesson $lesson)
    {
        $lesson->load('lessonable');
        $supportsInline = $lesson->lessonable_type === TextContent::class;

        request()->validate([
            'inline_document' => $supportsInline ? 'nullable|file|mimes:pdf|max:20480' : 'nullable',
            'attachments' => 'nullable|array',
            'attachments.*' => 'file|max:20480',
        ]);

        if (! $supportsInline && request()->hasFile('inline_document')) {
            return back()->withErrors(['inline_document' => 'Seuls les modules texte acceptent un document intÃ©grÃ©.']);
        }
        ...
        try {
            ...
            return back()->with('success', 'Ressources mises Ã  jour avec succÃ¨s.');
        } catch (\Exception $e) {
            return back()->withErrors(['error' => 'Erreur lors de la mise Ã  jour des ressources: '.$e->getMessage()]);
        }
    }

    public function deleteResource(Formation $formation, Chapter $chapter, Lesson $lesson, LessonResource $resource)
    {
        if ($resource->lesson_id !== $lesson->id) {
            return redirect()->route('formateur.formation.chapters.index', $formation)
                ->withErrors(['error' => 'Impossible de trouver la ressource demandÃ©e pour ce module.']);
        }

        $this->deleteLessonResource($resource);

        return back()->with('success', 'Ressource supprimÃ©e avec succÃ¨s.');
    }
        ...
        try {
            ...
            if (request()->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Titre de la leÃ§on modifiÃ© avec succÃ¨s!',
                    'new_title' => $validated['lesson_title'],
                ]);
            }

            return redirect()->route('formateur.formation.show', $formation)
                ->with('success', 'Titre de la leÃ§on modifiÃ© avec succÃ¨s!');
        } catch (\Exception $e) {
    ...
    /**
     * Persist an uploaded file as a lesson resource.
     */
    private function storeLessonResource(Lesson $lesson, UploadedFile $file, string $displayMode): LessonResource
    {
    ...
    /**
     * Remove all inline (auto displayed) documents linked to the lesson.
     */
    private function removeInlineResources(Lesson $lesson): void
    {
        $lesson->resources()
     ...
     */
    private function deleteLessonResource(LessonResource $resource): void
    {
